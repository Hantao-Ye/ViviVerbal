    ...
    def __init__(self, filename=None, filelist=None,
        ...
        if filelist:
            ...
            for eachfile in filelist:
                ...
                if eachfile[-4:] == 'xlsx':
                    ...
                    data = pd.read_csv('words/'+eachfile, sep=csvsep, header=None,
                                       names=['English', 'Chinese'], error_bad_lines=False)
        ...
        if filename:
            ...
            if filename[-4:] == 'xlsx':
                ...
                self.data = pd.read_csv('words/'+filename, sep=csvsep, header=None, names=[
                                        'English', 'Chinese'], error_bad_lines=False)
...
#            return _wrapper
    ...
    def main(self, data):
        ...
        print("本次单词范围%s~%s,%s/%s" %
              (self.start, self.end, len(order), word_num))
        print("==========================")
        for i in order:
            # 打印单词
            word_count += 1
            ...
            if input1 == 'q' or i == order[-1]:
                ...
                if self.score:
                    print('本次共检测%s个单词。正确%s个，错误%s个，正确率%.2f' %
                          (right+wrong, right, wrong, right/(right+wrong)*100))
                    if wrong == 0 and right >= 5:
                        ...
                        if not self.wrongflag:
                            ...
                            if self.save == True:
                                print("正在写入excel")
                                self.wrong.to_csv(
                                    'words/wrong.csv', index=False, header=False, sep='\t')
